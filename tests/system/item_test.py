...
class ItemTest(BaseTest):
    ...
    def test_create_item(self):
        with self.app() as client:
            with self.app_context():
                ...
                response = client.post('/item/test_item', data={'price': 17.99, 'store_id': 1})
    ...
    def test_create_duplicate_item(self):
        with self.app() as client:
            with self.app_context():
                StoreModel('test').save_to_db()
                client.post('/item/test_item', data={'price': 17.99, 'store_id': 1})
                response = client.post('/item/test_item', data={'price': 17.99, 'store_id': 1})
...
#                self.assertDictEqual({'name': 'test', 'items': []}, json.loads(response.data))
    ...
    def test_put_item(self):
        with self.app() as client:
            with self.app_context():
                StoreModel('test').save_to_db()
                response = client.put('/item/test_item', data={'price': 17.99, 'store_id': 1})
    ...
    def test_item_list(self):
        with self.app() as client:
            with self.app_context():
                ...
                self.assertDictEqual({'item': [{'name': 'test', 'price': 5.99}]},
                                     json.loads(response.data))
